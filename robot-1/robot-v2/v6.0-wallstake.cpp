...
motor rightMotorBack = motor(PORT14, ratio18_1, false);
...
unsigned short wallStakeArmPos = 0; // 0 = resting state, 1 =  enabled state, 2 = extended state
...
void initalize(void) {
    ...
    wallStakeArm.setVelocity(75, percent);
...
void controlWallStakeArm() {
    previousWallStakeArmPos = wallStakeArmPos;

    if (Controller.ButtonUp.pressing() && wallStakeArmPos < 2) {
        wallStakeArmPos += 1;
    }
    if (Controller.ButtonDown.pressing() && wallStakeArmPos > 0) {
        wallStakeArmPos -= 1;
    }

    if (wallStakeArmPos == 0) {
      if (previousWallStakeArmPos > wallStakeArmPos) {
        wallStakeArm.spinFor(reverse, 95, degrees);
      }

    } else if (wallStakeArmPos == 1) {
      if (previousWallStakeArmPos > wallStakeArmPos) {
        wallStakeArm.spinFor(reverse, 540, degrees);
      } else if (previousWallStakeArmPos < wallStakeArmPos) {
        wallStakeArm.spinFor(forward, 95, degrees);
      }

    } else if (wallStakeArmPos == 2) { 
      if (previousWallStakeArmPos < wallStakeArmPos) {
        wallStakeArm.spinFor(forward, 540, degrees);
      }
