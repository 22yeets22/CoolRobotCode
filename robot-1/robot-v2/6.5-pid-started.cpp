...
// Stuff for PID
double forwardKP = 0.01;
double forwardKI = 0.0;
double forwardKD = 0.0;
...
int forwardError;
int forwardPrevError = 0;
int forwardDerivative;
int forwardTotalError = 0;
...
void driveForwardPID(int targetDegrees) {
    while (PIDEnabled) {
        ...
        forwardError = averagePosition - targetDegrees;

        forwardDerivative = forwardError - forwardPrevError;

        forwardTotalError += forwardError;

        double motorPower = (forwardKP * forwardError) + (forwardKI * forwardTotalError) + (forwardKD * forwardDerivative);

        leftDriveSmart.spin(forward, motorPower, voltageUnits::volt);
        rightDriveSmart.spin(forward, motorPower, voltageUnits::volt);
...
void turnPID(int targetDegrees, int direction) {
    while (PIDEnabled) {
        int leftDrivePosition = leftDriveSmart.position(degrees);
        int rightDrivePosition = rightDriveSmart.position(degrees);
        int turnDifference = leftDrivePosition - rightDrivePosition;

        turnError = turnDifference - targetDegrees;

        turnDerivative = turnError - turnPrevError;

        turnTotalError += turnError;

        double motorPower = (turnKP * turnError) + (turnKI * turnTotalError) + (turnKD * turnDerivative);
